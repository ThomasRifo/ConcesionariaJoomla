 ...
 * @subpackage Helpers
 * @author Max Milbers
 * @copyright Copyright (c) 2011-2024 VirtueMart Team. All rights reserved by the author.
...
class VmMediaHandler {
	...
	var $file_url = '';
	var $file_url_folder = '';
	var $file_path_folder = '';
	var $file_url_thumb = '';
	var $file_name_thumb = '';
	var $file_url_folder_thumb = '';

	var $file_url_thumb_img = '';
	var $file_url_img = '';
	...
	function __construct($id=0){
		...
		$this->setRealImageSize = VmConfig::get('setRealImageSize',1);
	}
	...
	static public function createMedia($table,$type='',$file_mimetype=''){
		...
		$isImage = $table->is_image!==null ? $table->is_image: self::isImage($table->file_url);
	...
	function imageArgsToArray($imageArgs){
		...
		if(!empty($this->file_class)){
			...
			//vmdebug('filterImageArgs $this->file_class added',$imageArgs);
		}
		return $imageArgs;
	}
	...
	function displayMediaThumb($imageArgs=array(),$lightbox=true,$effect="rel='group'",$return = true,$withDescr = false,$absUrl = false, $width=0,$height=0){
		...
		if( substr( $this->file_url, 0, 2) == "//" ) {
			...
			try {
				...
				if($resObj->code!=200){
					vmdebug('displayMediaThumb URL does not exists',$toChk,$resObj);
		...
		if ($this->setRealImageSize) {
			if(empty($width) and !empty($imageArgs['width'])){
				$width = $imageArgs['width'];
			}
			if(empty($height) and !empty($imageArgs['height'])){
		...
		if ($this->setRealImageSize and (empty($width) or empty($height))) {
			if (file_exists($file_url_thumb) and !is_dir($this->file_url) ) { // Spiros
				//vmStartTimer('thumbs');
				//vmSetStartTime('thumbs1');
				$sizeAttr = getimagesize($file_url_thumb);
				$imageArgs['width'] = $sizeAttr[0];
				$imageArgs['height'] = $sizeAttr[1];
				//vmTime('Thumping','thumbs');
				//vmTime('Thumping1','thumbs1');
			}
		} else {
	...
	function displayIt($file_url, $file_alt, $imageArgs, $lightbox, $effect ="class='modal'",$withDesc=false,$absUrl = false){
		...
		if ($this->setRealImageSize and (empty($imageArgs['width']) or empty($imageArgs['height'])) and self::isImage($this->file_url)) { // Spiros
...
//			quorvia modified to give text in the images and allow jquery 1.13.2 to function May 2023
			if ($image->file_url_thumb > "0" ) {
				$htmlImages .= '<div class="vm_thumb_image">';
				$htmlImages .= '<div class="q_select_image_name" style="line-height:100%;font-size:9px;"> '.$image->file_title.'</div>';
				$htmlImages .= '<span> '.JHtml::image($image->file_url_thumb,$image->file_title, ' title="'.$image->file_title.'" class="vm_thumb" ').'</span>';
...
<td>';
		...
		$html .= '<td class="labelcell" style="width:20em">
	<label for="published">'. vmText::_('COM_VIRTUEMART_FILES_FORM_FILE_ISIMAGE') .'</label>
</td>
<td>';
		$html .= VmHtml::checkbox('media[is_image]', $this->is_image,1,0,'class="inputbox"','media[is_image]') ;
		$html .='</td>';
