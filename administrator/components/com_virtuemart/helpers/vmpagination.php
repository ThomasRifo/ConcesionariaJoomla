...
* @subpackage Helpers
* @author Max Milbers
* @copyright Copyright (c) 2011-2024 VirtueMart Team. All rights reserved.
...
class VmPagination extends vObject {
	...
	function getLimitBox ($sequence = 0) {
		...
		if(!VmConfig::isSite()) {
			...
			$link = vmURI::getCurrentUrlBy('get',false,true, array('limit'));
			if(empty($sequence)) {
			...
			if(empty($limits) or !is_array( $limits )) {
				...
				$perRow = intval($this->_perRow);
				$limits[$perRow*5] = JHtml::_( 'select.option', JRoute::_( $link.'&limit='.$perRow*5, false ), $perRow*5 );
				$limits[$perRow*10] = JHtml::_( 'select.option', JRoute::_( $link.'&limit='.$perRow*10, false ), $perRow*10 );
				$limits[$perRow*20] = JHtml::_( 'select.option', JRoute::_( $link.'&limit='.$perRow*20, false ), $perRow*20 );
				$limits[$perRow*50] = JHtml::_( 'select.option', JRoute::_( $link.'&limit='.$perRow*50, false ), $perRow*50 );
			}
			if(!array_key_exists( $this->limit, $limits )) {
	...
	public function getPagesLinks()
	{
		...
		$catId = vRequest::getInt('virtuemart_category_id',false);
		$itemId = vRequest::getInt('Itemid',false);
		if($itemId){
			$this->_additionalUrlParams['Itemid'] = $itemId;

			static $menu = null;
			if(!isset($menu)){
				$q = 'SELECT * FROM #__menu WHERE id = "'.$itemId.'"';
				$db = JFactory::getDbo();
				$db->setQuery($q);
				$menu = $db->loadObject();
				if($menu){

					$link = substr($menu->link,strpos($menu->link,'?')+1);
					$itemQuery = array();
					parse_str($link,$itemQuery);

					vmdebug('vmPagination getPagesLinks getMenu ',$catId,$itemQuery);
					if(!empty($itemQuery['virtuemart_category_id']) and $itemQuery['virtuemart_category_id'] == $catId){
						$catId = 0;
					}
		...
		if($catId) {
			...
			$this->_additionalUrlParams['virtuemart_category_id'] = $catId;
			vmdebug('_buildDataObject $this->_additionalUrlParams virtuemart_category_id',$catId);
		}
		...
		$chromePath = VMPATH_THEMES . '/' . $template->template . '/html/pagination.php';
		$parentChromePath = !empty($template->parent) ? VMPATH_THEMES . '/' . $template->parent . '/html/pagination.php' : null;
vmdebug('getPagesLinks',$chromePath);
		...
		elseif ($parentChromePath && file_exists($parentChromePath))
		{
			include_once $parentChromePath;
	...
	public function getListFooter($lbox=true)
	{
		...
		if ($parentChromePath && file_exists($parentChromePath))
		{
			include_once $parentChromePath;
	...
	protected function _buildDataObject()
	{
		...
		{
			$data->all->base = '0';
			$data->all->link = JRoute::_($params . $nextDelim . $this->prefix . 'limitstart=');
		}
		...
		if ($this->get('pages.current') > 1)
		{
			...
			$data->start->base = '0';
			$data->start->link = JRoute::_($params . $nextDelim . $this->prefix . 'limitstart=0');
			$data->previous->base = $page;
			$data->previous->link = JRoute::_($params . $nextDelim . $this->prefix . 'limitstart=' . $page);
		}
		...
		if ($this->get('pages.current') < $this->get('pages.total'))
		{
			...
			$data->next->base = $next;
			$data->next->link = JRoute::_($params . $nextDelim . $this->prefix . 'limitstart=' . $next);
			$data->end->base = $end;
			$data->end->link = JRoute::_($params . $nextDelim . $this->prefix . 'limitstart=' . $end);
		}
		...
		for ($i = $this->get('pages.start'); $i <= $stop; $i++)
		{
			...
			if ($i != $this->get('pages.current') || $this->_viewall)
			{
				...
				$data->pages[$i]->link = JRoute::_($params . $nextDelim . $this->prefix . 'limitstart=' . $offset);
			}
		}
		return $data;
	}
