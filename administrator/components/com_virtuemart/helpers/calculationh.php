 ...
 * @subpackage Helpers
 * @author Max Milbers
 * @copyright Copyright (c) 2010 - 2023 VirtueMart Team and the authors. All rights reserved.
...
class calculationHelper {
	...
	static $_calcModel = null;
	protected $customfieldsModel;

	var $rules = array();
	var $_roundindig = null;
	var $optimisedCalcSql = null;
	...
	static public function getInstance(int $vendorId = 1,int $countryId = 0,int $stateId = 0) {

		if (!isset(self::$_instance[$vendorId][$countryId][$stateId]) or !is_object(self::$_instance[$vendorId][$countryId][$stateId])) {
			self::$_instance[$vendorId][$countryId][$stateId] = new calculationHelper($vendorId, $countryId, $stateId);
		} else {
			//We store in UTC and use here of course also UTC
			$jnow = JFactory::getDate();
			self::$_instance[$vendorId][$countryId][$stateId]->_now = $jnow->toSQL();
		}
		return self::$_instance[$vendorId][$countryId][$stateId];
	...
	public function setVendorId($id){
		...
		if(empty(self::$_calcModel)){
			self::$_calcModel = VmModel::getModel('calc');
		}
		...
		if(empty($this->allrules[$this->productVendorId])){
			...
			$q .= $shopperGrpJoin . $countryGrpJoin . $stateGrpJoin . 'ORDER BY c.ordering';
	...
	public function getProductPrices(&$product, $variant=0.0, $amount=0) {
		...
		//$this->basePrice =
		$basePriceShopCurrency = $this->productPrices['basePrice'] = !empty($basePriceMargin) ? $basePriceMargin : $basePriceShopCurrency;
		...
		if(!empty($this->rules['VatTax'])){
			...
			foreach($this->rules['VatTax'] as &$rule){
				if(isset($this->_cart->cartData['VatTax'][$rule['virtuemart_calc_id']])){
					...
					$this->_cart->cartData['VatTax'][$rule['virtuemart_calc_id']]['taxAmount'] += $this->productPrices['taxAmount'] * $amount;
					$this->_cart->cartData['VatTax'][$rule['virtuemart_calc_id']]['subTotal']  += $this->productPrices['salesPrice'] * $amount;

				} else {
					$this->_cart->cartData['VatTax'][$rule['virtuemart_calc_id']] = $rule;
					if(!isset($this->_cart->cartData['VatTax'][$rule['virtuemart_calc_id']]['taxAmount'])) $this->_cart->cartData['VatTax'][$rule['virtuemart_calc_id']]['taxAmount'] = $this->productPrices['taxAmount'] * $amount;
					if(!isset($this->_cart->cartData['VatTax'][$rule['virtuemart_calc_id']]['subTotal'])) $this->_cart->cartData['VatTax'][$rule['virtuemart_calc_id']]['subTotal'] = $this->productPrices['salesPrice'] * $amount;
				}
			}
		}
	...
	public function getCheckoutPrices(&$cart) {
		...
		$this->_cart->cartData['couponCode'] = $this->_cart->couponCode;
...
//When the rule got not active, we remove the rule
		...
		if (!empty($this->_cart->cartData['couponCode'])) {
			$this->couponHandler($this->_cart->cartData['couponCode']);
		}
		...
		if($this->_currencyDisplay->_priceConfig['salesPrice']){
			...
			if($this->_cart->cartData['vmVat'] and (!empty($cartdiscountBeforeTax) and isset($this->_cart->cartData['VatTax']) and count($this->_cart->cartData['VatTax'])>0) or !empty($this->_cart->cartData['couponCode'])){
	...
	protected function getCalcRuleData($calcRuleID) {
		...
		$calcRule = self::$_calcModel->getCalc($calcRuleID);
		return $calcRule;
	...
	protected function couponHandler($_code) {
		...
		if(!empty($_data->virtuemart_product_ids) or !empty($_data->virtuemart_category_ids)){
			...
			foreach($this->_cart->products as $i=>$product){
				if( (!empty($allowed_product_ids) and in_array($product->virtuemart_product_id,$allowed_product_ids))
					or (!empty($allowed_productcat_ids and !empty($product->categories)) and array_intersect($product->categories, $allowed_productcat_ids))){
	...
	function gatherEffectingRulesForProductPrice($entrypoint, $id) {
		...
		foreach ($this->allrules[$this->productVendorId][$entrypoint] as $i => &$rule) {
			...
			if ($hitsCategory and $hitsShopper and $hitsDeliveryArea and $hitsManufacturer) {
				if ($this->_debug)
					vmdebug ('Add rule ForProductPrice at entrypoint '.$entrypoint.' '. $rule["calc_name"]);
	...
	function gatherEffectingRulesForBill($entrypoint, $cartVendorId=1) {
		...
		foreach ($rules as $rule) {
			...
			$rule = (array)self::$_calcModel->getCalc($rule['virtuemart_calc_id']);
