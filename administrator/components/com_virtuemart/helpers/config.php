...
class VmConfig {
	...
	static function showDebug($override=false){
		...
		if(self::$_debug===NULL or $override!=false){
			...
			vmEcho::$debugSet = vmEcho::$_debug = self::$_debug;
			if ($dev === 'all') {
				self::setErrRepDebug();
			} else if($dev === 'none'){
	...
	static function setErrRepDebug(){
		...
		if(version_compare($cVer,'8.0.0','<' ) ){
			$ret[1] = error_reporting( E_ALL );
		} else {
			$ret[1] = error_reporting( E_ALL & ~E_DEPRECATED);
		}
...
/**
	...
	*/
	...
	static $iniLang = true;
	static protected $execTrigger = true;
...
//In WP, we run the define, when we render vm, in Joomla we have to run them here
	...
	static public function isSite(){
		...
		if(self::$isSite===null){
			...
			if(!self::isSiteByApp() or vmAccess::isManagingFE(0)){
				self::$isSite = false;
			} else {
				self::$isSite = true;
			}
