...
class VirtuemartViewPaymentMethod extends VmViewAdmin {
	...
	function display($tpl = null) {
		...
		if ($layoutName == 'edit') {
			...
			$currency = CurrencyDisplay::getInstance($payment->virtuemart_vendor_id);
			//$currency = VirtueMartModelVendor::getVendorCurrency ($payment->virtuemart_vendor_id);
			$this->assignRef('vendor_currency', $currency->_symbol);

			if(empty($payment->currency_id)) $payment->currency_id = $currency->_currency_id;
