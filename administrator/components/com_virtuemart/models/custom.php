...
class VirtueMartModelCustom extends VmModel {
    ...
    function getCustoms($custom_parent_id,$search = false){
	    ...
	    $datas = new stdClass();
		$datas = $this->exeSortSearchListQuery(0, $query, '',$whereString,$this->_getOrdering());
	...
	function getCustomsList ($publishedOnly = true) {
		...
		foreach($result as $key=>$custom){
			if($uniqueCustomfieldtitles == '1'){
				$expl_title = explode(' - ',$custom->text);
				$result[$key]->text = vmText::_($expl_title[0]).' - '.$expl_title[1];
			}else{
				$result[$key]->text = vmText::_($custom->text);
			}
		}
...
//vmdebug('Storing customplugin',$data);
		...
		} else if ( is_array($data['virtuemart_shoppergroup_id']) ){
