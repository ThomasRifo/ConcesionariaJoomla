...
<div class="custom_products" class="ui-sortable">'.   $prodStr .'</div>';
				...
				foreach( $product->customfields as $k => $productCustom ) {
					...
					if(isset($product->cart_item_id)) {
						...
						if($pluginFields == false and isset($product->customPlugin)) {
							//vmdebug('calculateModificators ',$product->customPlugin);
							if(is_string($product->customPlugin)){
								$pluginFields = vmJsApi::safe_json_decode( $product->customPlugin, TRUE );
							} else {
								$pluginFields = $product->customPlugin;
							}
	...
	*/
	...
	static public function setEditCustomHidden ($customfield, $i) {
		...
		if(!empty($customfield->custom_element)){
			$html .= '<input type="hidden" value="' . $customfield->custom_element . '" name="field[' . $i . '][custom_element]" />';
		}
		if(!empty($customfield->custom_jplugin_id)){
			$html .= '<input type="hidden" value="' . $customfield->custom_jplugin_id . '" name="field[' . $i . '][custom_jplugin_id]" />';
		}
