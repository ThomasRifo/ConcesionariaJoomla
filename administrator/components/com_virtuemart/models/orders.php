 ...
 * @version $Id: orders.php 11097 2024-11-21 20:42:45Z Milbo $
 */
		...
		foreach($order['items'] as $p=>$item){
			...
			if($product){
				...
				foreach ( $pvar as $k => $v) {
					...
					if (!isset($item->{$k}) and property_exists($product, $k)) {
						$item->{$k} = $v;
					}
				}
			}
		...
		if ( $orderUpdate and !empty($data['virtuemart_order_item_id'])) {
			...
			if(!empty($data['virtuemart_product_id']) and empty($data['order_item_name'])){
				...
				if(empty($orderUserId)){
					...
					$orderuser_shoppergroups = array($std_grp->virtuemart_shoppergroup_id);
...
//vmdebug('updateSingleItem $taxCalcValue',$taxCalcValue,$data);
	...
	function calculateRow($data, $taxCalcValue, $rounding, $daTax = true, $withTax = true, $overrideDiscount = false){
		...
		$data['product_final_price'] = doubleval($data['product_final_price']);
		$data['product_subtotal_discount'] = doubleval($data['product_subtotal_discount']);
		$data['product_basePriceWithTax'] = doubleval($data['product_basePriceWithTax']);
		$data['product_item_price'] = doubleval($data['product_item_price']);
		$taxValue = doubleval($taxCalcValue);
		...
		if(VirtueMartModelOrders::isNotEmptyDec($data,'product_item_price')){
			...
			$data['product_basePriceWithTax'] = round( doubleval($data['product_item_price']) * (1 + $taxValue/100.0), $rounding );
			...
			if($daTax){
				if($overrideDiscount and VirtueMartModelOrders::isEmptyDec($data,'product_subtotal_discount') and VirtueMartModelOrders::isNotEmptyDec($data,'product_final_price')){
					$itemDiscount = $data['product_basePriceWithTax'] - doubleval($data['product_final_price']);
				} else {
...
//$data['product_subtotal_discount'] = (round($orderdata->product_final_price, $rounding) - round($data['product_basePriceWithTax'], $rounding)) * $orderdata->product_quantity;
		$data['product_subtotal_with_tax'] = round($data['product_final_price'], $rounding) * round((float)$data['product_quantity'],3);
...
//vmdebug('my prices',$data);
	...
	function toggle($field,$val = NULL, $cidname = 0,$tablename = 0, $view = false  ) {
		...
		foreach($ids as $id){
			...
			if($field == 'paid'){
				...
				//	quorvia force toggling to invoice unlocked regardless
				if (!empty(VmConfig::get('unlockInvoices',false)) ) {
					if($field == "invoice_locked") {
						$val = '0';
					}
				}
	...
	public function updateBill($virtuemart_order_id, /*$vattax,*/ $order_rules = false){
		...
		if (!empty( $virtuemart_order_id)) {
			...
			vmdebug('$calculate_billTaxAmount $bill_rules',$calculate_billTaxAmount,$bill_rules);
/*			if(!empty($bill_rules))
			{
			...
			*/
			...
			$date = JFactory::getDate();
			$today = $date->toSQL();
			vmdebug('Updated order $calculate_billTaxAmount ',$calculate_billTaxAmount);
			if(empty($calculate_billTaxAmount)){
				//shipment
				...
				if ( $os!="" )
				{
					...
					vmdebug('Updated order shipment ',$sql);
				}
				...
				if ( $op!="" )
				{
					...
					vmdebug('Updated order payment ',$sql);
				}
			}
	...
	function safeUpdateOrderStatus ($orderOrId, $orderstatus, $allowed = array('P')) {

		if(is_int($orderOrId)){
			$order = $this->getOrder($orderOrId);
			$virtuemart_order_id = $orderOrId;
		} else {
			$order = $orderOrId;
			$virtuemart_order_id = $order['details']['BT']->virtuemart_order_id;
		}

		if(!empty($order['details']['BT']) and $order['details']['BT']->order_status != $orderstatus and in_array($order['details']['BT']->order_status, $allowed)){
			$oData['order_status'] = $orderstatus;
			$this->updateStatusForOneOrder($virtuemart_order_id, $oData);
			return true;
		}
		return false;
	}
...
/*				if( (/*count($summarizedRules['taxBill'])==1 or * VmConfig::get('radicalShipPaymentVat',true)) and isset($summarizedRules['taxBill'][$idWithMax])){
			...
			//	quorvia dont lock my invoice, // Milbo I think we dont need that
			if (!empty(VmConfig::get('unlockInvoices',false))) {
				$data->invoice_locked = 0;
			}
			$data->store();
		...
		//vmdebug('Notify customer calling todo this data is almost the same, we should merge and so we do not need to load again the just stored order ', $data, $inputOrder);
		//We must first store the history, because the email uses getOrder to load the data
		$orderHistoryTable = $this->updateOrderHistory($inputOrder);
		$customer_notified = $this->notifyCustomer( $data->virtuemart_order_id , $inputOrder );
		if($customer_notified and $orderHistoryTable){
			$orderHistoryTable->customer_notified = 1;
			$orderHistoryTable->check();
			$orderHistoryTable->store();
		}
		...
		if(empty($data->paidSet)){
			...
			if(in_array($data->order_status,$os_trigger_paid)){
				...
				foreach ($itms as $id => $item) {
					if(!$item){
						vmdebug('calculatePaidByOS $item is false ',$inputOrder);
						continue;
					}
					$os_trigger_refunds = VmConfig::get('os_trigger_refunds', array('R'));
					...
					if(in_array($order_status,$os_trigger_refunds)){
						//$ok = false;
						//VmInfo('Cannot set order to paid/unpaid, because there are refunded items. Please check the order manually '.$id);
						$data->paid -= $item->product_final_price * $item->product_quantity;
					}
				}
	...
	private function _createOrder($_cart, $_usr)
	{
		...
		if (!empty($_cart->cartData['couponCode'])) {
			$_orderData->coupon_code = $_cart->cartData['couponCode'];
		...
		if (!empty($_cart->cartData['couponCode'])) {
			...
			CouponHelper::setInUseCoupon($_cart->cartData['couponCode'], true);
		}
		...
		//VmEcho::$logDebug = 1;
		//vmdebug('_createOrder', $_cart->virtuemart_order_id, $orderTable->virtuemart_order_id);
		return $orderTable->virtuemart_order_id;
	}

	function deleteOldPendingOrder($cart){

		$reUseTimeSql = VmConfig::get('reuseorders',0);vmdebug('deleteOldPendingOrder '.$reUseTimeSql);
		...
		if(!empty($cart->virtuemart_order_id)){
			$q .= ' `virtuemart_order_id`= "'.$cart->virtuemart_order_id.'" ';
			$q .= 'AND (`order_status` = "P" OR `order_status` = "U")';
		} else if($cart->customer_number){
			$q .= ' `customer_number`= "'.$cart->customer_number.'" ';
			$q .= 'AND `order_status` = "P" ';
			$jnow = JFactory::getDate();
			$minusT =  date('Y-m-d H:i:s', strtotime('-'.$reUseTimeSql.' minutes', strtotime($jnow)));
			$q .= 'AND `created_on` > "'.$minusT.'" ';
		} else {
			return false;
		}
		...
		vmdebug('deleteOldPendingOrder',$virtuemart_order_id);
		if($virtuemart_order_id) {
			$this->remove(array($virtuemart_order_id),false);
	...
	function handleStockAfterStatusChangedPerProduct($newState, $oldState, $tableOrderItems, $quantity) {
		...
		if (!empty($tableOrderItems->product_attribute)) {
			...
			$product_attributes = vmJsApi::safe_json_decode( $tableOrderItems->product_attribute, true );
...
//quorvia end
		...
		if($sendMail){
			if(!empty($vars['orderDetails']['details']) and !empty($vars['orderDetails']['details']['BT']->order_language)) {
				...
				if(VmConfig::$vmlangTag!=$orderLang){
	...
	public function getInvoiceIfAvailable($order){
		...
		//vmdebug('getInvoiceIfAvailable start',$order );
		$inv = false;
		$invoiceNumberDate = array();
		$force_create_invoice=vRequest::getInt('create_invoice', -1);
		$invM = VmModel::getModel('invoice');
		//TODO we need an array of orderstatus
		if ( /*/empty($order['details']['BT']->invoice_locked) and */($force_create_invoice==1 or $invM->isInvoiceToBeAttachByOrderstats($order['details']['BT']->order_status)) ){
	 ...
	 *
	 */
	function removePSDataOfOrder($orderId, $auth = true){
	...
	public function remove($ids, $auth = true) {
		...
		foreach($ids as $id) {
			...
			$this->removePSDataOfOrder($id, $auth);
	...
	public function UpdateOrderHead($virtuemart_order_id, $_orderData) {
		...
		foreach ($_userFieldsBT as $_fld) {
			...
			if(isset( $_orderData["BT_{$_name}"])){
				...
				$_userInfoData[$_name] = $_userFieldsModel->prepareFieldDataSave($_fld, $_orderData, 'BT_');
			}
		}
		...
		foreach ($_userFieldsST as $_fld) {
			...
			if(isset( $_orderData["ST_{$_name}"])){
				...
				$_userInfoData[$_name] = $_userFieldsModel->prepareFieldDataSave($_fld, $_orderData,'ST_');
			}
		}
