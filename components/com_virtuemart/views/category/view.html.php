...
class VirtuemartViewCategory extends VmView {
	...
	public function display($tpl = null) {
		...
		'productsublayout' => VmConfig::get('productsublayout', ''),
			...
			foreach( $opt as $o ) {
				...
				if($o == 'products') {
					...
					if(!empty( $this->{$o} ) and !empty( $this->{$o.'_rows'} )) {
						$perRow = intval($this->perRow);
						$this->products[$o] = $this->productModel->getProductListing( $o, $perRow * $this->{$o.'_rows'} );
		...
		if ($this->products) {
			...
			$showCustoms = (int)VmConfig::get('show_pcustoms',1);
...
//		quorvia get rid of any excess data
	...
	public function setTitleByJMenu(){
		...
		//@todo I think this sequence should be different
