...
class VirtuemartViewUser extends VmView {
	...
	function display($tpl = null) {
		...
		if(!$user->guest){
			...
			if(!empty($params->get('allowUserRegistration')) and $this->address_type == 'BT'){
				...
				if(VmConfig::get('regNeedProdCart', true) and empty($this->cart->cartProductsData)){
					$this->showRegisterText = false;
					$this->showRegistration = false;
					vmdebug('I dont show register because regNeedProdCart empty($this->cart->cartProductsData)');
				}
			} else {
				vmdebug('I dont show register because either no registration allowed by joomla, or ST',$params->get('allowUserRegistration'));
			}
