 ...
 * @version $Id: user.php 11099 2024-12-02 11:59:33Z Milbo $
 */
...
class VirtueMartControllerUser extends JControllerLegacy {
	...
	function editAddressCart(){
		...
		$view = $this->getView('user', 'html', '', array('layout' => 'edit_address'));
	 ...
	 */
	public function saveData($cartObj) {
		...
		//We store the entered data always in the cart
		$cart = VirtueMartCart::getCart();
		$prefix= '';
		...
		//guests who do not want to register are finished
		if(!$register and $currentUser->guest and $cartObj and ($cartObj->_fromCart or $cartObj->getInCheckOut()) ) {
			return true;
		}
		...
		if($data['address_type'] == 'ST' and !$currentUser->guest){
			...
			if(/*$register and */$currentUser->guest and VmConfig::get('regNeedProdCart', true)){
				//$cart = VirtueMartCart::getCart();
			...
			//if($currentUser->guest!=1 or !$cartObj or  ($currentUser->guest==1 and $register) ){
				...
				if($switch and !empty($ret['newId'])){ //and VmConfig::get ('oncheckout_change_shopper')){
					//update session
	...
	function removeAddressST(){
		...
		if(is_array($virtuemart_user_id)){
			$virtuemart_user_id = reset($virtuemart_user_id);
		}

		$userModel = VmModel::getModel('user');
		vmdebug('removeAddressST',$virtuemart_user_id,$virtuemart_userinfo_id);

		$userModel->removeAddress($virtuemart_userinfo_id, $virtuemart_user_id);
