		...
		foreach($customfields as $key => $customfield){
			...
			switch ($type) {
						...
						foreach($variants as $k => $variant){
							...
							if(!in_array($variant,$dropdowns[$k])  ){
								...
								if($k==0 or !$productSelection){
									...
									if(is_array($productSelection) and is_array($variants)){
										...
										vmdebug('\com_virtuemart\sublayouts\customfield.php not array',$productSelection,$variants );
									}
				...
				case 'PB':
					...
					$view = vRequest::getCmd('view','category');

					if($view != 'productdetails' and isset($customfield->orderableBrowse)){

						$product->orderable = !$customfield->orderableBrowse;
					}
